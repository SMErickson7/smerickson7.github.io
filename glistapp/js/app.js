    ...
    .then(
      function (snapshot) {
        ...
        $(aisles).each(function (i, e) {
          ...
          $(
            '<div class="three columns"><div onclick="toggleList(' +
              (i + 1) +
              ", '" +
              aisles[i].item.toLocaleLowerCase().replace(" & ", "_") +
              "-list" +
              '\')"><div class="icon"><i class="toggle" id="toggle' +
              (i + 1) +
              '"></i></div><div class="header"><h5 id="toggleSwitch">' +
              aisles[i].item +
              '</h5></div></div><div id="' +
              aisles[i].item.toLocaleLowerCase().replace(" & ", "_") +
              "-list" +
        ...
        for (var j = 0; j < aisles.length; j++) {
          ...
          var divAisle =
            varCategory.toLocaleLowerCase().replace(" & ", "_") + "-list";
          for (var i = 0; i < gData[varCategory].length; i++) {
            food = $(
              '<label class="container-checkbox">' +
                gData[varCategory][i].item +
                "<input onclick=\"addToList('" +
                gData[varCategory][i].item +
                '\')" id="' +
                gData[varCategory][i].item +
                '" value="' +
      ...
      function (snapshot) {
        ...
        $(function () {
          ...
          $("#tags")
            ...
            return $("<li>")
              .append(
                "<div>" + makeBold($("#tags").val(), item.label) + "</div>"
              )
              .appendTo(ul);
          };
        });
      ...
      firebase
        ...
        .then(
          function (snapshot) {
            ...
            if (gList == null) {
              document.getElementById("mydiv").innerHTML =
                "No items have been added to the grocery list";
            }
