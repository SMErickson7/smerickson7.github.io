    ...
    .then(
      function (snapshot) {
        ...
        $(aislesX).each(function (i, e) {
          ...
          $(
            '<div class="three columns"><div class="header"><h5 contenteditable="true">' +
              aislesX[i].item +
              '</h5></div><ul class="sortable connectedSortable" id="' +
              aislesX[i].item.toLowerCase().replace(" & ", "_") +
              '-list"  data-aisle="' +
              aislesX[i].item +
              '" ></ul></div>'
          ).appendTo(container);
          aislesID.push(
            "#" + aislesX[i].item.toLowerCase().replace(" & ", "_") + "-list"
          );
        ...
        for (var j = 0; j < aislesX.length; j++) {
          ...
          for (var i = 0; i < gData[aisleName].length; i++) {
            li = $(
              '<li class="ui-state-default" data-groceryitem="' +
                gData[aisleName][i].item +
                '" data-category="' +
                aisleName +
                '">' +
                gData[aisleName][i].item +
                '<a href="#" class="clicked" data-item="' +
                gData[aisleName][i].item +
                '" data-aisle="' +
                aisleName +
                '"><div class="removeItem"><i class="fa-solid fa-xmark"></i></div></a></li>'
            );
            li.data("d", gData[aisleName][i]);
            $(
              "#" + aisleName.toLocaleLowerCase().replace(" & ", "_") + "-list"
            ).append(li);
          }
        }
        $(".clicked").click(function () {
          ...
          var aisleID =
            "#" + removeFromAisle.toLowerCase().replace(" & ", "_") + "-list";
          var aisleArray = gData[removeFromAisle];
          var removeItemPosition = aisleArray
            .map(function (e) {
              return e.item;
            })
            .indexOf(removeItem);
  ...
  firebase
    ...
    .then(function (snapshot) {
      ...
      firebase
        ...
        .set({ item: d[0].item });
      li = $(
        '<li class="ui-state-default" data-category=' +
          category +
          ">" +
          d[0].item +
          "</li>"
      ...
      li.data("d", update);
      $(
        "#" + category.toLocaleLowerCase().replace(" & ", "_") + "-list"
      ).append(li);
    });
}

//Get the button:
mybutton = document.getElementById("settingsScroll");
