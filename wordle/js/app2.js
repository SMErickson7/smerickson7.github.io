document.addEventListener("DOMContentLoaded", () => {
    ...
    function getFinalWordArr() {
        return guessedWords[wLength + 1 - 1];
    }

    function updateGuessedWords(letter) {
        const currentWordArr = getCurrentWordArr();

        if (currentWordArr && currentWordArr.length < wLength) {
    ...
    function handleSubmitWord() {
        ...
        if (currentWordArr.length !== wLength) {}
        ...
        if (!words.includes(currentWord)) {
            const firstLetterId = guessedWordCount * wLength + 1;
        ...
        const firstLetterId = guessedWordCount * wLength + 1;
        const interval = 200;
        if (guessedWords.length != wLength + 1) {
            if (currentWord === word) {
                currentWordArr.forEach((letter, index) => {
                    setTimeout(() => {
        ...
        //FINAL WRONG WORD
        if (guessedWords.length === wLength + 1) {
            const firstLetterId = wLength * wLength + 1;
            streak = 0;
            reveal = word.split("");
